...
export class EventDataProvider {
  ...
  removeMessagePic(key: any): PromiseLike<any> {
  ...
  createMessage(userName: string, message: string, picture = null, roomId: string, userId: string, pic: string, time): PromiseLike<any> {
    return this.messageRooms.child(roomId).child('messageList').push({
      userId: userId,
      name: userName,
      text: message,
      pic: pic,
  ...
  createEvent(eventName: string, eventDate: string, eventPrice: number,
    eventCost: number): PromiseLike<any> {
  ...
  //add item
  addItem(userName, itemName, itemPrice, desc, itemPic = null, yourId): PromiseLike<any> {
  ...
  removePubItem(itemId): PromiseLike<any> {
    return this.pubItems.child(itemId).remove();
  }
  //delete your item also in pub
  deleteItem(itemId): PromiseLike<any> {
  ...
  addGuest(guestName, eventId, eventPrice, guestPicture = null): PromiseLike<any> {

...
export class AuthDataProvider {
   ...
   */
  loginUser(email: string, password: string): PromiseLike<any> {
   ...
   * @param  {string} password [User's password]
   */
  signupUser(email: string, password: string): PromiseLike<any> {
   ...
   * @param  {string} email    [User's email address]
   */
  resetPassword(email: string): PromiseLike<any> {
